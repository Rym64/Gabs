...
/********************** Search seance **************************/
Route::GET('/searchseance',function(){
  $seance = Input::get ( 'seance' );
  $user = User::where('date', '%Y-%m-%d','LIKE','%'.$seance.'%')->where('Heure_debut', '%Y-%m-%d','LIKE', '%'.$seance.'%')->where('Heure_fin', '%Y-%m-%d','LIKE','%'.$seance.'%')->get();
  if(count($seance) > 0)
      return view('teacher.abscense.affectabsence')->withDetails($seance)->withQuery ( $seance );
  else return view ('teacher.abscense.affectabsence')->withMessage('No Details found. Try to search again !');
});
...
Route::middleware('auth')->group(function () {
    ...
    //Route::get('teacher/chercherseance', 'Teacher\SeanceController@chercherseance');
